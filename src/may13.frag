...
float sdfPlane(vec3 pt, vec3 n, float d){
    return dot(pt,n)-d;//内積=dot(,)
}

float sdf(vec3 pt){
    float d1 = sdfSphere(pt, vec3(0.0,0.3,0.0),0.1*radius);
...
vec3 sdfNormal(vec3 pt){
    ...
    float fy = (sdf(pt+vec3(0.0,eps, 0))-sdf(pt))/eps;
    float fz = (sdf(pt+vec3(0.0, 0.0, eps))-sdf(pt))/eps;
...
void main(){
    ...
    float depth = 7.0;
    vec3 rightdir = cross(cdir, updir);//cdir=カメラが向いている方向、updirは上面、rightdirはカメラ右側が向いている方向
    vec3 ray = p.x*rightdir + p.y*updir + depth*cdir;//視線のベクトルの設定
    ray = normalize(ray);//長さを１にする

    vec3 pt = rayMarching(camera, ray);

    if(abs(sdf(pt))<0.01){
        vec3 lightray = normalize(light - pt);//法線
        float intensity = dot(lightray, sdfNormal(pt));//内積を求める
